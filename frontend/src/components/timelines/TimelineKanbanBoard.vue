  ...
  <div class="px-4 pb-8">
    <div class="mb-6 flex flex-wrap items-center gap-4">
      <div class="flex-1 min-w-50">
        <label class="block text-sm font-medium text-slate-600 mb-2">選擇專案</label>
        <select :value="selectedKanbanTimeline" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none shadow-sm" @change="$emit('update:selected-kanban-timeline', parseNullableNumber($event))">
          <option :value="null">📁 全部專案</option>
          <option v-for="t in timelines" :key="t.id" :value="t.id">📋 {{ t.name }}</option>
        </select>
      </div>
      <div class="flex-1 min-w-50">
        <label class="block text-sm font-medium text-slate-600 mb-2">搜尋任務</label>
        <div class="relative">
          <input :value="searchQuery" type="text" placeholder="輸入任務名稱..." class="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none shadow-sm" @input="$emit('update:search-query', ($event.target as HTMLInputElement).value)" />
          <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">🔍</span>
        </div>
      </div>
      <div class="flex items-end gap-2">
        <button @click="$emit('toggle-filter-panel')" :class="['px-4 py-2.5 rounded-xl border transition-all flex items-center gap-2 shadow-sm', hasActiveFilters ? 'bg-linear-to-r from-primary to-blue-600 text-white border-transparent' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50 hover:border-slate-300']">
          <span>🎯</span> 篩選
    ...
    <div v-if="showFilterPanel" class="mb-6 p-5 bg-linear-to-r from-white to-slate-50/50 rounded-2xl border border-slate-200 shadow-lg">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div>
          <label class="block text-sm font-medium text-slate-600 mb-2">優先級</label>
          <select :value="filterPriority" class="w-full px-3 py-2.5 border border-slate-200 rounded-xl bg-white shadow-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none" @change="$emit('update:filter-priority', parseNullableNumber($event))">
        ...
        <div>
          <label class="block text-sm font-medium text-slate-600 mb-2">標籤</label>
          <input :value="filterTag" type="text" placeholder="輸入標籤關鍵字..." class="w-full px-3 py-2.5 border border-slate-200 rounded-xl bg-white shadow-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none" @input="$emit('update:filter-tag', ($event.target as HTMLInputElement).value)" />
        </div>
        <div class="flex items-end">
          <button @click="$emit('clear-filters')" class="px-4 py-2.5 text-slate-500 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all flex items-center gap-2">
            <span>🗑️</span> 清除篩選
          </button>
        </div>
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div class="bg-linear-to-b from-slate-100 to-slate-50 rounded-2xl p-4 shadow-sm">
        ...
        <draggable :list="pendingTasks" group="kanban" item-key="task_id" :animation="200" ghost-class="kanban-ghost" drag-class="kanban-drag" @start="$emit('drag-start')" @end="$emit('drag-end')" @change="$emit('pending-change', $event)" class="space-y-3 min-h-50">
          <template #item="{ element: task }">
            <TaskCard :task="task" column="pending" :get-priority-badge-class="getPriorityBadgeClass" :get-priority-label="getPriorityLabel" :get-subtask-progress="getSubtaskProgress" :get-completed-subtask-count="getCompletedSubtaskCount" :get-task-timeline-name="getTaskTimelineName" :format-date-fn="formatDateFn" @open="$emit('open-task', task)" />
          </template>
        </draggable>
        <div v-if="pendingTasks.length === 0 && !isDragging" class="text-center py-12 text-slate-400">
      ...
      <div class="bg-linear-to-b from-blue-100 to-blue-50 rounded-2xl p-4 shadow-sm">
        ...
        <draggable :list="inProgressTasks" group="kanban" item-key="task_id" :animation="200" ghost-class="kanban-ghost" drag-class="kanban-drag" @start="$emit('drag-start')" @end="$emit('drag-end')" @change="$emit('in-progress-change', $event)" class="space-y-3 min-h-50">
          <template #item="{ element: task }">
            <TaskCard :task="task" column="in_progress" :get-priority-badge-class="getPriorityBadgeClass" :get-priority-label="getPriorityLabel" :get-subtask-progress="getSubtaskProgress" :get-completed-subtask-count="getCompletedSubtaskCount" :get-task-timeline-name="getTaskTimelineName" :format-date-fn="formatDateFn" @open="$emit('open-task', task)" />
          </template>
        </draggable>
        <div v-if="inProgressTasks.length === 0 && !isDragging" class="text-center py-12 text-slate-400">
          <span class="text-3xl mb-2 block">🚀</span>
          <span class="text-sm">拖曳任務到這裡</span>
        </div>
      </div>
      ...
      <div class="bg-linear-to-b from-green-100 to-green-50 rounded-2xl p-4 shadow-sm">
        ...
        <draggable :list="completedTasks" group="kanban" item-key="task_id" :animation="200" ghost-class="kanban-ghost" drag-class="kanban-drag" @start="$emit('drag-start')" @end="$emit('drag-end')" @change="$emit('completed-change', $event)" class="space-y-3 min-h-50">
          <template #item="{ element: task }">
            <TaskCard :task="task" column="completed" :get-priority-badge-class="getPriorityBadgeClass" :get-priority-label="getPriorityLabel" :get-subtask-progress="getSubtaskProgress" :get-completed-subtask-count="getCompletedSubtaskCount" :get-task-timeline-name="getTaskTimelineName" :format-date-fn="formatDateFn" @open="$emit('open-task', task)" />
          </template>
        </draggable>
        <div v-if="completedTasks.length === 0 && !isDragging" class="text-center py-12 text-slate-400">
          <span class="text-3xl mb-2 block">🎉</span>
  ...
  template: `
    <div @click="$emit('open', task)" :class="[
      ...
      <div class="flex items-start justify-between mb-2">
        <span :class="['font-medium text-sm line-clamp-2', column==='completed' ? 'text-slate-500 line-through' : 'text-slate-800']">{{ task.name }}</span>
      ...
      <div class="flex items-center justify-between text-xs text-slate-500 pt-2 border-t border-slate-200">
        <span :class="[
          ...
          column==='pending' ? 'bg-slate-100' : ''
        ]">📅 {{ formatDateFn(task.end_date) }}</span>
