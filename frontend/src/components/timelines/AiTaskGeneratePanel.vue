<template>
  <div class="p-6">
    <div v-if="isGeneratingAi" class="text-center py-12">
      ...
      <p class="text-slate-600 font-medium">AI 正在生成任務建議...</p>
      <p class="text-slate-400 text-sm mt-2">請稍候，正在分析專案內容</p>
    </div>

    <div v-else-if="aiGeneratedTasks.length === 0" class="py-8">
      <p class="text-slate-500 mb-4 text-center">可輸入需求情境，讓 AI 透過 MCP 生成更貼近專案的任務建議</p>
      <div class="space-y-3 mb-5">
        <label class="block text-sm font-medium text-slate-700">需求描述（可選）</label>
        <textarea
          :value="aiPrompt"
          rows="4"
          placeholder="例如：這個月要完成登入流程重構，請拆成後端 API、前端頁面、測試與上線準備"
          class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none resize-none"
          @input="$emit('update:ai-prompt', ($event.target as HTMLTextAreaElement).value)"
        ></textarea>
        <div class="flex flex-wrap items-center gap-4 text-sm text-slate-600">
          <label class="inline-flex items-center gap-2 cursor-pointer select-none">
            <input :checked="useRagPlanning" type="checkbox" class="w-4 h-4 rounded border-slate-300 text-primary focus:ring-primary" @change="$emit('update:use-rag-planning', ($event.target as HTMLInputElement).checked)" />
            使用 RAG 規劃建議
          </label>
          <label v-if="!useRagPlanning" class="inline-flex items-center gap-2 cursor-pointer select-none">
            <input :checked="useCopilotMcp" type="checkbox" class="w-4 h-4 rounded border-slate-300 text-primary focus:ring-primary" @change="$emit('update:use-copilot-mcp', ($event.target as HTMLInputElement).checked)" />
            優先使用 AI + MCP 工具路由
          </label>
          <label v-if="useRagPlanning" class="inline-flex items-center gap-2 cursor-pointer select-none">
            <input :checked="usePersonalKnowledge" type="checkbox" class="w-4 h-4 rounded border-slate-300 text-primary focus:ring-primary" @change="$emit('update:use-personal-knowledge', ($event.target as HTMLInputElement).checked)" />
            納入個人知識庫
          </label>
          <label v-if="useRagPlanning" class="inline-flex items-center gap-2 cursor-pointer select-none">
            <input :checked="useProjectKnowledge" type="checkbox" class="w-4 h-4 rounded border-slate-300 text-primary focus:ring-primary" @change="onProjectKnowledgeToggle" />
            納入專案檔案
          </label>
          <label class="inline-flex items-center gap-2 cursor-pointer select-none">
            <input :checked="autoCreateAfterGenerate" type="checkbox" class="w-4 h-4 rounded border-slate-300 text-primary focus:ring-primary" @change="$emit('update:auto-create-after-generate', ($event.target as HTMLInputElement).checked)" />
            生成後直接建立任務
          </label>
        </div>
      ...
      <div class="text-center">
        <button @click="$emit('generate')" class="rounded-xl bg-primary px-6 py-3 font-semibold text-white shadow-[0_8px_18px_rgba(37,99,235,0.24)] transition-all hover:brightness-110">
    ...
    <div v-else class="space-y-4">
      <div class="flex items-center justify-between mb-4">
        <p class="text-sm text-slate-600">共 {{ aiGeneratedTasks.length }} 個建議任務，已選 {{ selectedAiTasks.length }} 個</p>
        <div class="flex gap-2">
          ...
          <button @click="$emit('reset-generated')" class="text-sm text-slate-400 hover:text-slate-600">重新生成</button>
        </div>
      </div>
      ...
      <div class="space-y-3 max-h-80 overflow-y-auto">
        <div
          ...
          :class="['p-4 rounded-xl border-2 cursor-pointer transition-all', selectedAiTasks.includes(index) ? 'border-primary/40 bg-primary/5' : 'border-slate-200 hover:border-slate-300']"
        >
          <div class="flex items-start gap-3">
            <div :class="['w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 mt-0.5', selectedAiTasks.includes(index) ? 'border-primary bg-primary' : 'border-slate-300']">
            ...
            <div class="flex-1">
              ...
              <p v-if="task.remark" class="text-sm text-slate-500 mt-1">{{ task.remark }}</p>
            </div>
          </div>
        </div>
      </div>
      <div class="flex gap-3 pt-2">
        <button @click="$emit('close')" class="flex-1 py-2.5 border border-slate-200 text-slate-600 font-medium rounded-xl hover:bg-slate-50 transition-colors">取消</button>
        <button @click="$emit('batch-create')" :disabled="selectedAiTasks.length === 0" :class="['flex-1 py-2.5 font-semibold rounded-xl transition-all', selectedAiTasks.length > 0 ? 'bg-primary text-white hover:brightness-110 shadow-[0_8px_18px_rgba(37,99,235,0.24)]' : 'bg-slate-100 text-slate-400 cursor-not-allowed']">
