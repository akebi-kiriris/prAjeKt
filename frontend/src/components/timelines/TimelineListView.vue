    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
      <div class="p-4 border-b border-slate-200 bg-slate-50/50">
        <div class="flex items-center justify-between">
          <h3 class="font-semibold text-slate-700">📋 專案列表</h3>
          <span class="text-sm text-slate-500">依結束日期排序</span>
        </div>
      </div>
      <div class="divide-y divide-slate-100">
        ...
        >
          <div class="flex items-start gap-4">
            <div class="shrink-0 w-20 text-center">
              ...
              <p class="text-xs text-slate-400 mt-1">{{ getDaysRemaining(timeline.endDate).text }}</p>
            </div>
            <div class="flex-1 min-w-0">
              <div class="flex items-start justify-between gap-2 mb-2">
                <h4 class="font-semibold text-slate-800 truncate">{{ timeline.name }}</h4>
              ...
              <div class="mb-2">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs font-medium text-slate-500 w-10 text-right">{{ getTaskProgress(timeline) }}%</span>
                </div>
              </div>
        ...
        <div v-if="timelinesCount === 0" class="text-center py-16">
          ...
          <p class="text-lg text-slate-600">目前尚無專案</p>
          <p class="text-sm text-slate-400 mt-1">點擊「新增專案」來建立您的第一個專案</p>
        </div>
      </div>
    </div>
  </div>
