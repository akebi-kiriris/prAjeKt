        ...
        <div class="p-5">
          <div class="flex justify-between items-start mb-4">
            <div class="flex-1 min-w-0">
              <h3 class="font-semibold text-slate-800 truncate mb-1">{{ timeline.name }}</h3>
            ...
            <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity" @click.stop>
              <button v-if="timeline.role === 0" @click="$emit('edit-timeline', timeline)" class="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-primary hover:bg-primary/10 rounded-lg transition-colors">✏️</button>
              <button v-if="timeline.role === 0" @click="$emit('delete-timeline', timeline.id)" class="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">🗑️</button>
            </div>
          </div>
          <div class="flex items-center justify-between mb-4 p-3 bg-slate-50 rounded-xl">
            <div class="flex items-center gap-2">
              ...
              <div>
                <p class="text-xs text-slate-500">剩餘時間</p>
            ...
            <div class="text-right">
              ...
              <p class="text-sm font-medium text-slate-700">{{ formatDate(timeline.endDate) || '未設定' }}</p>
            </div>
          </div>
          <div class="mb-4" v-if="timeline.startDate && timeline.endDate">
            ...
            <p class="text-xs text-slate-400 text-center mt-1">時程進度 {{ getTimeProgress(timeline) }}%</p>
          </div>
          <div class="mb-4">
            <div class="flex justify-between text-sm mb-1">
              <span class="text-slate-500">任務完成度</span>
          ...
          <div class="flex items-center justify-between pt-3 border-t border-slate-200">
            <div class="flex items-center gap-2 text-xs text-slate-400"><span>📅 {{ formatDate(timeline.startDate) || '未設定' }}</span></div>
            <span class="text-xs text-primary font-medium group-hover:underline">查看詳情 →</span>
          </div>
        </div>
      </div>
    </div>
    <div v-if="timelinesCount === 0" class="text-center py-16 bg-white rounded-2xl border border-dashed border-slate-200">
      <span class="text-6xl block mb-4">📁</span>
      <p class="text-xl text-slate-600 mb-2">目前尚無專案</p>
      <p class="text-sm text-slate-400 mb-6">建立您的第一個專案來開始追蹤進度</p>
