<template>
  <div class="p-6 space-y-6">
    <div class="grid grid-cols-2 gap-4 p-4 bg-slate-50 rounded-xl">
      <div><p class="text-xs text-slate-500 mb-1">開始日期</p><p class="font-medium text-slate-800">{{ formatDate(selectedTask.start_date) || '未設定' }}</p></div>
      <div><p class="text-xs text-slate-500 mb-1">截止日期</p><p class="font-medium text-slate-800">{{ formatDate(selectedTask.end_date) || '未設定' }}</p></div>
    </div>
    <div v-if="selectedTask.task_remark" class="p-4 bg-yellow-50 rounded-xl">
      <h4 class="font-semibold text-slate-700 mb-2">📝 備註</h4>
      <p class="text-slate-600 text-sm">{{ selectedTask.task_remark }}</p>
    </div>
    <div class="p-4 bg-slate-50 rounded-xl">
      <h4 class="font-semibold text-slate-700 mb-2">🔗 前置依賴</h4>
      <select
    ...
    <div v-if="canManageTaskMembers(selectedTask)" class="p-4 bg-indigo-50/60 rounded-xl">
      ...
      <div v-if="taskMembersForAssign.length > 0" class="flex flex-wrap gap-2 mb-3">
        <div
          ...
          :class="member.role === 0 ? 'bg-primary/20 text-primary' : 'bg-white border border-slate-200 text-slate-700'"
        >
          <span>{{ member.name }}</span>
          <span class="text-slate-400 text-[10px]">{{ member.role === 0 ? '負責人' : '協作者' }}</span>
          <button
            v-if="member.role !== 0"
          ...
          <button
            ...
            class="ml-0.5 text-slate-400 hover:text-red-500 transition-colors leading-none"
          >✕</button>
        </div>
      </div>
      <div v-else class="text-xs text-slate-400 mb-3">尚未指派任何成員</div>
      <div v-if="timelineMembers.length > 0">
        <p class="text-xs text-slate-500 mb-2">快速指派專案成員：</p>
        <div class="flex flex-wrap gap-2">
          ...
          <span v-if="timelineMembers.filter(m => !taskMembersForAssign.some(tm => tm.user_id === m.user_id)).length === 0" class="text-xs text-slate-400">所有成員已加入</span>
        </div>
      </div>
    </div>
    ...
    <div>
      <h4 class="font-semibold text-slate-700 mb-3 flex items-center gap-2">
        <span>📋</span> 子任務
        <span class="text-sm font-normal text-slate-500">({{ taskSubtasks.filter(s => s.completed).length }}/{{ taskSubtasks.length }})</span>
      </h4>
      <div v-if="taskSubtasks.length > 0" class="h-2 bg-slate-200 rounded-full overflow-hidden mb-4">
        <div class="h-full bg-primary rounded-full transition-all duration-300" :style="{ width: subtaskProgress + '%' }"></div>
      </div>
      <div class="space-y-2 mb-3">
        <div v-for="subtask in taskSubtasks" :key="subtask.id" class="flex items-center gap-3 p-3 bg-slate-50 rounded-lg group hover:bg-slate-100 transition-colors">
        ...
        <div v-if="taskSubtasks.length === 0" class="text-center py-4 text-slate-400 text-sm">尚無子任務</div>
      </div>
    ...
    <div>
      <div class="flex items-center justify-between mb-3">
        <h4 class="font-semibold text-slate-700 flex items-center gap-2">
          <span>📎</span> 附件
          <span class="text-xs text-slate-400 font-normal">({{ taskFiles.length }})</span>
        </h4>
      ...
      <div v-if="taskFiles.length === 0" class="text-center py-6 text-slate-400 text-sm bg-slate-50 rounded-xl border border-dashed border-slate-200">
        尚無附件，點擊「上傳檔案」新增
      </div>
      <div v-else class="space-y-2">
        <div v-for="file in taskFiles" :key="file.id" class="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-200 hover:bg-slate-100 transition-colors group">
          <img v-if="isImageFile(file.original_filename)" :src="`${apiBaseUrl}/tasks/files/${file.filename}`" class="w-12 h-12 object-cover rounded-lg border border-slate-200 shrink-0" :alt="file.original_filename" />
          <span v-else class="text-3xl shrink-0">{{ getFileIcon(file.original_filename) }}</span>
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-slate-700 truncate">{{ file.original_filename }}</p>
    ...
    <div>
      <div class="mb-4 flex items-center justify-between gap-3">
        <h4 class="font-semibold text-slate-700 flex items-center gap-2">
          <span>💬</span> 留言
          <span class="text-xs text-slate-400 font-normal">({{ taskComments.length }})</span>
        </h4>
      ...
      <div v-if="commentSummary" class="mb-4 p-4 bg-violet-50 border border-violet-100 rounded-xl text-sm text-slate-700 space-y-3">
        <div>
      ...
      <div class="space-y-3 max-h-60 overflow-y-auto mb-4">
        <div v-for="comment in taskComments" :key="comment.comment_id" class="flex gap-3 p-3 bg-slate-50 rounded-xl group">
          ...
          <button @click="$emit('delete-comment', comment.comment_id)" class="opacity-0 group-hover:opacity-100 w-7 h-7 flex items-center justify-center text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all shrink-0" title="刪除留言">✕</button>
        </div>
        <div v-if="taskComments.length === 0" class="text-center py-4 text-slate-400 text-sm">尚無留言</div>
      </div>
      <div class="flex gap-2">
        <input :value="newComment" type="text" placeholder="新增留言..." @input="$emit('update:new-comment', ($event.target as HTMLInputElement).value)" @keyup.enter="$emit('add-comment')" class="flex-1 px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none" />
