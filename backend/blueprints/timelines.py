...
def _pydantic_error_message(err: ValidationError):
    ...
    if base == "start_date 格式錯誤":
        return "開始日期格式錯誤"
    if base == "end_date 格式錯誤":
        return "結束日期格式錯誤"
    return base
...
def create_timeline():
    ...
    if error:
        return error
    data, error = _validate_payload_or_400(TimelineCreateRequest, data)
    if error:
        return error
...
def update_timeline(timeline_id):
    ...
    if error:
        return error
    data, error = _validate_payload_or_400(TimelineUpdateRequest, data)
    if error:
        return error
...
def check_timeline_conflict(timeline_id):
    ...
    if error:
        return error
    data, error = _validate_payload_or_400(ConflictCheckInput, data)
    if error:
        return error
...
def update_timeline_remark(timeline_id):
    ...
    if error:
        return error
    data, error = _validate_payload_or_400(TimelineRemarkRequest, data)
    if error:
        return error
...
def search_user_by_email():
    ...
    if error:
        return error
    data, error = _validate_payload_or_400(TimelineSearchUserRequest, data)
    if error:
        return error
...
def add_timeline_member(timeline_id):
    ...
    if error:
        return error
    data, error = _validate_payload_or_400(TimelineAddMemberRequest, data)
    if error:
        return error
...
def generate_tasks_with_ai(timeline_id):
    ...
    if error:
        return error
    data, error = _validate_payload_or_400(TimelineGenerateTasksRequest, data)
    if error:
        return error
...
def batch_create_tasks(timeline_id):
    ...
    if error:
        return error
    data, error = _validate_payload_or_400(TimelineBatchCreateTasksRequest, data)
    if error:
        return error
